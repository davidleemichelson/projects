...
string Board::getLabel(int row, int col) {
	return _board[row][col].getSquareLetter();
}
...
string Board::doMove(Move* move, bool* validCommand, Bag* bag, Player* player, Dictionary* dict) {
	...
	// Evaluate Exchange command
		...
		if(move->getRow() > _y || move->getCol() > _x || move->getRow() < 1 || move->getCol() < 1) {
			...
			*validCommand = false;
			return "One or more of the tiles placed would be out of bounds of the board. Please try again.\n";
		}
		...
		if(player->playerHasTiles(placeLetters, "PLACE")) { // if the player has the tiles he/she wishes to place
			...
			if(_board[_starty][_startx].getSquareLetter() == "***") { // First move
				if (placeLetters.size() == 1) {
					...
					*validCommand = false;
					return "One letter words are not valid first moves. Please try again.";
				}
				...
				if(move->getPlaceDirection() == "-") { 
					if(row != y || (col + wordSize - 1) < x || col > x) { // if not in the same row as the start or if the word won't reach the start
						...
						*validCommand = false; // reprompt for move in scrabble.cpp
						return "For the first move of the game, one of your tiles must be on the start square.\nPlease try another move.";
					}
					else {
						if(dict->contains(placeLetters)) { //  Don't have to check other tiles; first move. Go straight to dictionary check.
							...
							*validCommand = false;
							return "Word " + placeLetters + " not found in dictionary. Please try again.";
						}
					}
				}
				...
				if(_board[row-1][col-1].isCovered()) { // This if statement checks if the player is trying to place on a tiled square
					...
					cout << "Please try again." << endl;
					*validCommand = false;
					return "The square you selected is already covered by a tile. Please try again.";
				}
				// If the player desires to make a horizontal move not on the first turn
					...
					if(!dict->contains(eastWestPlaceLetters)) {
						...
						*validCommand = false;
						return "Word " + eastWestPlaceLetters + " not found in dictionary. Please try again.";
					}
					...
					for(unsigned int i = 0; i < placeLetters.size(); i++) { // need to use a loop since we are not just checking two ends (E/X)
						...
						if(!dict->contains(northSouthPlaceLetters)) {
							...
							*validCommand = false;
							return "Word " + northSouthPlaceLetters + " not found in dictionary. Please try again.";
						}
						else {
					...
					if(!adjacent) {
						...
						*validCommand = false;
						return "At least one tile must be placed adjacent to a tile already on the board. Please try again.";
					} 
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////				
					...
					for(unsigned int i = 0; i < placeLetters.size(); i++) { // need to use a loop since we are not just checking two ends (E/X)
						...
						if(!dict->contains(northSouthPlaceLetters)) {
							...
							*validCommand = false;
							return "Invalid word would be formed from placement. Please try again";
						}
						else {
							if(northSouthPlaceLetters.size() == 1) { // meaining no north/south neighbors. onto the next one. this watched out for "a"
								continue;
			...
			cout << "Please try again." << endl;
			*validCommand = false;
			return "You must have the tiles that you wish to place.";
		}
		*validCommand = true;
		return "";
	}
		...
		return "Did not recognize your command, please try again.";
	}
	return "";
}
