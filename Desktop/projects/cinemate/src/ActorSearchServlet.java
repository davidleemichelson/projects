...
//		System.out.println("Testing if data works Current logged in user is: " + data.getLoggedInUser().getUsername());
		...
		if(data.searchByActor(searchInput.toLowerCase()) != null) {
			searchResultSet = data.searchByActor(searchInput.toLowerCase());
		}
		...
		request.setAttribute("ActorSearchResults", searchResultStringSet);
		request.getRequestDispatcher("actorSearch.jsp").forward(request, response);
