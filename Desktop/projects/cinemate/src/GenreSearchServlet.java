...
//		System.out.println("Testing if data works Current logged in user is: " + data.getLoggedInUser().getUsername());
		...
		if(data.searchByGenre(searchInput.toLowerCase()) != null) {
			searchResultSet = data.searchByGenre(searchInput.toLowerCase());
		}
		...
		request.setAttribute("GenreSearchResults", searchResultStringSet);
		request.getRequestDispatcher("genreSearch.jsp").forward(request, response);
