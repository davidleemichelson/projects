...
class ScoreAI : public BaseAI {
 public:
  ...
  Move getMove (const Board & board, const Player & player, std::map<char, int> initialTileCount) {
    ...
    int hiScoreIndex; int hiScore = 0;
    for(unsigned int i = 0; i < _possibleMoves.size(); i++) {
      int dummyScore = 0;
      board.getWords(*_possibleMoves[i], dummyScore);
      if (dummyScore > hiScore) {
        hiScore = dummyScore;
        hiScoreIndex = i;
        // std::cout << "New max score " <<hiScore<< " at " << hiScoreIndex <<std::endl;
      }
    }
    return *_possibleMoves[hiScoreIndex];    // now, find which move is the best move
